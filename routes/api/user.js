...
const { post } = require('../../../logmicro/routes/api/log');

const db = mysql.createConnection(config.dbConnection);
const redisClient = redis.createClient(config.redisConfiguration);
...
router.get('/list', (req, res) => {
    let sql = "SELECT * FROM user ORDER BY id DESC LIMIT 10";
    promiseGetRedis("userlist").then(result => {
        ...
        db.query(sql, function(errors, users) {
            ...
            setRedis("userlist", JSON.stringify(users), config.redisExpiredSec);
...
router.post('/login', (req, res) => {
    ...
    console.log("ABCCCCC: ", req.route.path);
